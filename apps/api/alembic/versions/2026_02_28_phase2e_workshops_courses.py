"""Phase 2E — Workshops & Courses

New tenant tables: courses, course_sessions, course_enrollments, course_session_attendance.
...
def _apply_to_tenant(schema: str):
    """Add workshop/course tables to existing tenant schemas."""
    op.execute(f"""
    ...
    """)
    op.execute(f"""
        CREATE TABLE IF NOT EXISTS {schema}.course_sessions (
            id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    ...
    """)
    op.execute(f"""
        CREATE TABLE IF NOT EXISTS {schema}.course_session_attendance (
            id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    ...
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_courses_status ON {schema}.courses(status);")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_courses_type ON {schema}.courses(type);")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_csess_course ON {schema}.course_sessions(course_id);")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_cenroll_course ON {schema}.course_enrollments(course_id);")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_cenroll_member ON {schema}.course_enrollments(member_id);")
...
def upgrade():
    ...
    # Update provision function — add course tables
...
def downgrade():
    for schema in _get_tenant_schemas():
        op.execute(f"DROP TABLE IF EXISTS {schema}.course_session_attendance;")
        op.execute(f"DROP TABLE IF EXISTS {schema}.course_enrollments;")
        op.execute(f"DROP TABLE IF EXISTS {schema}.course_sessions;")
        op.execute(f"DROP TABLE IF EXISTS {schema}.courses;")


def _provision_function_sql():
    """Full provision function including course tables."""
    return """
