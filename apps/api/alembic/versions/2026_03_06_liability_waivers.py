"""Liability waiver tables (tenant-scoped)

Adds waiver_templates and waiver_signatures tables to each tenant schema.
Updates provision_tenant_schema() for new tenants.

Revision ID: liability_waivers_001
Revises: ai_token_billing_001
Create Date: 2026-03-06
"""
import sqlalchemy as sa
from alembic import op

revision = "liability_waivers_001"
down_revision = "ai_token_billing_001"
branch_labels = None
depends_on = None


def _apply_to_tenant(schema: str) -> None:
    """Apply waiver tables to a single tenant schema."""

    # ── waiver_templates ──────────────────────────────────────────────
    op.execute(f"""
    CREATE TABLE IF NOT EXISTS {schema}.waiver_templates (
    ...
    # ── waiver_signatures ─────────────────────────────────────────────
    op.execute(f"""
    CREATE TABLE IF NOT EXISTS {schema}.waiver_signatures (
        id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
        waiver_template_id  UUID NOT NULL REFERENCES {schema}.waiver_templates(id) ON DELETE CASCADE,
    ...
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_wsig_member_template ON {schema}.waiver_signatures(member_id, waiver_template_id)")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_wsig_member_expires ON {schema}.waiver_signatures(member_id, expires_at)")
    op.execute(f"CREATE INDEX IF NOT EXISTS idx_{safe}_wtpl_active ON {schema}.waiver_templates(is_active) WHERE is_active = TRUE")


def upgrade():
    # 1. Apply to all existing tenant schemas
    conn = op.get_bind()
    rows = conn.execute(
    ...
    # 2. Update provision function to include waiver tables
...
def downgrade():
    ...
    for row in rows:
        schema = row[0]
        op.execute(f"DROP TABLE IF EXISTS {schema}.waiver_signatures CASCADE")
        op.execute(f"DROP TABLE IF EXISTS {schema}.waiver_templates CASCADE")


def _provision_function_sql():
    """Full provision function including waiver tables."""
    return """
        ...
        BEGIN
            ...
            -- ── Waiver Templates ─────────────────────────────────────────
            EXECUTE format('
            CREATE TABLE IF NOT EXISTS %I.waiver_templates (
                id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                version         INTEGER NOT NULL DEFAULT 1,
                title           VARCHAR(255) NOT NULL,
                content         TEXT NOT NULL,
            ...
            -- ── Waiver Signatures ────────────────────────────────────────
            EXECUTE format('
            CREATE TABLE IF NOT EXISTS %I.waiver_signatures (
                id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            ...
            EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_wsig_member_template ON %I.waiver_signatures(member_id, waiver_template_id)', replace(p_schema_name, '-', '_'), p_schema_name);
            EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_wsig_member_expires ON %I.waiver_signatures(member_id, expires_at)', replace(p_schema_name, '-', '_'), p_schema_name);
            EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_wtpl_active ON %I.waiver_templates(is_active) WHERE is_active = TRUE', replace(p_schema_name, '-', '_'), p_schema_name);
