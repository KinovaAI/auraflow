"""Phase 2C — Private Sessions Enhancement

Add columns to private_bookings, add indexes for performance.
...
def _apply_to_tenant(schema: str):
    """Add new columns and indexes to existing tenant schemas."""
    op.execute(f"""
        ALTER TABLE {schema}.private_bookings
    ...
    op.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{safe}_avail_instructor_day
            ON {schema}.instructor_availability(instructor_id, day_of_week);
    """)
    op.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{safe}_pb_starts
            ON {schema}.private_bookings(starts_at);
    """)
    op.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{safe}_pb_instructor_starts
            ON {schema}.private_bookings(instructor_id, starts_at);
    """)
...
def upgrade():
    ...
    # Drop all overloaded versions of provision function, then recreate
    op.execute("DROP FUNCTION IF EXISTS af_global.provision_tenant_schema(TEXT, TEXT);")
    op.execute("DROP FUNCTION IF EXISTS af_global.provision_tenant_schema(VARCHAR, TEXT);")
    op.execute("DROP FUNCTION IF EXISTS af_global.provision_tenant_schema(UUID, TEXT);")
...
def downgrade():
    for schema in _get_tenant_schemas():
        ...
        """)
        safe = schema.replace("-", "_")
        op.execute(f"DROP INDEX IF EXISTS {schema}.idx_{safe}_avail_instructor_day;")
        op.execute(f"DROP INDEX IF EXISTS {schema}.idx_{safe}_pb_starts;")
        op.execute(f"DROP INDEX IF EXISTS {schema}.idx_{safe}_pb_instructor_starts;")
