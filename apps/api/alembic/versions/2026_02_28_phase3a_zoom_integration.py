"""Phase 3A — Zoom Livestream + Auto-Recording Pipeline

Adds Zoom S2S OAuth credential columns to af_global.organizations.
Adds is_virtual/auto_record to class_series, recording columns to class_sessions.
Updates videos source CHECK to include 'zoom_recording'.
...
def _apply_to_tenant(schema: str):
    """Add Zoom-related columns to existing tenant tables."""
    safe = schema.replace("-", "_")

    # Add is_virtual and auto_record to class_series
    op.execute(f"""
        ALTER TABLE {schema}.class_series
        ADD COLUMN IF NOT EXISTS is_virtual BOOLEAN DEFAULT FALSE,
        ADD COLUMN IF NOT EXISTS auto_record BOOLEAN DEFAULT FALSE;
    """)

    # Add recording columns to class_sessions
    op.execute(f"""
        ALTER TABLE {schema}.class_sessions
        ADD COLUMN IF NOT EXISTS auto_record BOOLEAN DEFAULT FALSE,
        ADD COLUMN IF NOT EXISTS recording_status VARCHAR(30) DEFAULT 'none',
        ADD COLUMN IF NOT EXISTS recording_url TEXT,
        ADD COLUMN IF NOT EXISTS video_id UUID;
    """)

    # Add CHECK constraint for recording_status
    op.execute(f"""
        DO $$
        BEGIN
            ALTER TABLE {schema}.class_sessions
            ADD CONSTRAINT chk_{safe}_recording_status
            CHECK (recording_status IN ('none','recording','processing','ready','published','failed'));
        EXCEPTION WHEN duplicate_object THEN NULL;
        END $$;
    """)

    # Add index on recording_status for sessions that have recordings
    op.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{safe}_sessions_recording_status
        ON {schema}.class_sessions(recording_status)
        WHERE recording_status != 'none';
    """)

    # Update videos source CHECK to include 'zoom_recording'
    # Drop old constraint and recreate
    op.execute(f"""
        DO $$
        BEGIN
            ALTER TABLE {schema}.videos DROP CONSTRAINT IF EXISTS videos_source_check;
            ALTER TABLE {schema}.videos
            ADD CONSTRAINT videos_source_check
            CHECK (source IN ('youtube', 'mux', 'manual', 'zoom_recording'));
...
def upgrade():
    # ── Global schema: Add Zoom credential columns to organizations ─────
    op.execute("""
        ALTER TABLE af_global.organizations
        ADD COLUMN IF NOT EXISTS zoom_account_id VARCHAR(100),
        ADD COLUMN IF NOT EXISTS zoom_client_id_encrypted BYTEA,
        ...
        ADD COLUMN IF NOT EXISTS zoom_auto_record BOOLEAN DEFAULT TRUE,
        ADD COLUMN IF NOT EXISTS zoom_auto_publish BOOLEAN DEFAULT FALSE;
    """)
...
def downgrade():
    for schema in _get_tenant_schemas():
        safe = schema.replace("-", "_")
        op.execute(f"ALTER TABLE {schema}.class_sessions DROP COLUMN IF EXISTS auto_record;")
        op.execute(f"ALTER TABLE {schema}.class_sessions DROP COLUMN IF EXISTS recording_status;")
        op.execute(f"ALTER TABLE {schema}.class_sessions DROP COLUMN IF EXISTS recording_url;")
        op.execute(f"ALTER TABLE {schema}.class_sessions DROP COLUMN IF EXISTS video_id;")
        op.execute(f"ALTER TABLE {schema}.class_series DROP COLUMN IF EXISTS is_virtual;")
        op.execute(f"ALTER TABLE {schema}.class_series DROP COLUMN IF EXISTS auto_record;")
        op.execute(f"DROP INDEX IF EXISTS {schema}.idx_{safe}_sessions_recording_status;")

        # Restore original videos source CHECK
        op.execute(f"""
            DO $$
            BEGIN
                ALTER TABLE {schema}.videos DROP CONSTRAINT IF EXISTS videos_source_check;
    ...
    op.execute("""
        ...
        DROP COLUMN IF EXISTS zoom_client_id_encrypted,
        DROP COLUMN IF EXISTS zoom_client_secret_encrypted,
        DROP COLUMN IF EXISTS zoom_connected_at,
        DROP COLUMN IF EXISTS zoom_webhook_secret_encrypted,
        DROP COLUMN IF EXISTS zoom_auto_record,
        DROP COLUMN IF EXISTS zoom_auto_publish;
    """)


def _provision_function_sql():
    """Full provision function including Zoom columns."""
    return """
