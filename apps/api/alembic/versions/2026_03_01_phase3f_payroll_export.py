"""Phase 3F — Payroll Export & Integrations

Adds Gusto/QuickBooks OAuth credential columns to af_global.organizations,
payroll_employee_mapping table per tenant, and exported_at/export_method
columns to payroll_runs.

Revision ID: a3f_pe01
Revises: s10_ai01
"""
...
def upgrade() -> None:
    ...
    # ── Global schema: Gusto + QuickBooks OAuth credentials ───────────
    conn.execute(sa.text("""
        ALTER TABLE af_global.organizations
        ADD COLUMN IF NOT EXISTS gusto_client_id_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS gusto_access_token_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS gusto_refresh_token_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS gusto_company_id VARCHAR(100),
        ADD COLUMN IF NOT EXISTS gusto_connected_at TIMESTAMPTZ,
        ADD COLUMN IF NOT EXISTS qb_client_id_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS qb_client_secret_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS qb_access_token_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS qb_refresh_token_encrypted BYTEA,
        ADD COLUMN IF NOT EXISTS qb_realm_id VARCHAR(100),
    ...
    for schema in _tenant_schemas(conn):
        ...
        # payroll_employee_mapping table
        conn.execute(sa.text(f"""
            CREATE TABLE IF NOT EXISTS "{schema}".payroll_employee_mapping (
                ...
                CONSTRAINT payroll_mapping_provider_check
                    CHECK (provider IN ('gusto', 'quickbooks')),
                CONSTRAINT payroll_mapping_unique
                    UNIQUE (instructor_id, provider)
            )
        """))
        conn.execute(sa.text(f"""
            CREATE INDEX IF NOT EXISTS idx_{safe}_payroll_mapping_provider
            ON "{schema}".payroll_employee_mapping (provider)
        """))

        # Add export tracking columns to payroll_runs
        conn.execute(sa.text(f"""
            ALTER TABLE "{schema}".payroll_runs
            ADD COLUMN IF NOT EXISTS exported_at TIMESTAMPTZ,
            ADD COLUMN IF NOT EXISTS export_method VARCHAR(20)
        """))
...
def downgrade() -> None:
    ...
    for schema in _tenant_schemas(conn):
        conn.execute(sa.text(f"""
            ...
            DROP COLUMN IF EXISTS export_method
        """))
        conn.execute(sa.text(f"""
            DROP TABLE IF EXISTS "{schema}".payroll_employee_mapping CASCADE
        """))

    conn.execute(sa.text("""
        ALTER TABLE af_global.organizations
        DROP COLUMN IF EXISTS gusto_client_id_encrypted,
        DROP COLUMN IF EXISTS gusto_access_token_encrypted,
        DROP COLUMN IF EXISTS gusto_refresh_token_encrypted,
        DROP COLUMN IF EXISTS gusto_company_id,
        DROP COLUMN IF EXISTS gusto_connected_at,
        DROP COLUMN IF EXISTS qb_client_id_encrypted,
        DROP COLUMN IF EXISTS qb_client_secret_encrypted,
        DROP COLUMN IF EXISTS qb_access_token_encrypted,
        DROP COLUMN IF EXISTS qb_refresh_token_encrypted,
        DROP COLUMN IF EXISTS qb_realm_id,
        DROP COLUMN IF EXISTS qb_connected_at
    """))


def _provision_function_sql():
    """Full provision function including payroll export tables."""
    return """
