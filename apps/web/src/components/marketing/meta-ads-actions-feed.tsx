...
import { metaAdsApi, type MetaAIAction } from "@/lib/meta-ads-api";
...
interface Props {
  actions: MetaAIAction[];
  pendingActions: MetaAIAction[];
}

export function MetaAdsActionsFeed({ actions, pendingActions }: Props) {
  const queryClient = useQueryClient();

  const approveMutation = useMutation({
    mutationFn: (id: string) =>
      metaAdsApi.approveAction(id).then((r) => r.data.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["meta-ads-actions"] });
      queryClient.invalidateQueries({ queryKey: ["meta-ads-pending"] });
    },
  });

  const rejectMutation = useMutation({
    mutationFn: (id: string) =>
      metaAdsApi.rejectAction(id).then((r) => r.data.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["meta-ads-actions"] });
      queryClient.invalidateQueries({ queryKey: ["meta-ads-pending"] });
    },
